        ...
        if (state.isComposing && !state.candidates.empty()) {
            QMetaObject::invokeMethod(this, [this, state]() {
                ...
                if (!m_candidateWindow->isWindowVisible()) {
                    QRect cursorRect(m_cursorPos.x(), m_cursorPos.y() - 20, 1, 20);
...
DWORD ServerApp::handleIPCRequest(const IPCMessage& msg, std::wstring& response) {
    switch (msg.cmd) {
        ...
        case IPC_UPDATE_POSITION: {
            int x = static_cast<int>(msg.param1);
            int y = static_cast<int>(msg.param2);
            QPoint newPos(x, y);
            QMetaObject::invokeMethod(this, [this, newPos]() {
                m_cursorPos = newPos;
